...
class Drone(db.model):
    ...
    cost_of_producton = db.Column(db.Number(presision=10, scale=2))
    series = db.Column(db.String(150))
    random_joke = db.Column(db.String, nullable=True)
    user_token = db.Column(db.String, db.ForeignKey('user.token'), nullable = False)

    def __init__(self, name, description, price, camera_quality, flight_time, max_speed, dimension, weight,
                 cost_of_producton, series, random_joke, user_token):
        self.id = self.set.id()
        self.name = name
        ...
        self.weight = weight
        self.cost_of_producton = cost_of_producton
        self.series = series
